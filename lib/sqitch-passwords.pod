...
A recommended method for secure Oracle connections is to use C<Oracle Wallets|https://docs.oracle.com/middleware/1213/wls/JDBCA/oraclewallet.htm#JDBCA596>.
This can be a bit more tricky to set up but results in the password never being stored in a readable format.
You can then configure sqitch to use this through a TNS entry.

  sqitch config --system target.mydatabase.uri 'db:oracle://:@/tns_alias'
